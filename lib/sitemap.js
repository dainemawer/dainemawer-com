...
const fs = require('fs');
const {generateAllArticles, generateAllProjects} = require('./helpers');
...
exports.generateSitemap = async (domain, targetFolder) => {
    ...
    const allArticles = await generateAllArticles();
    const allProjects = await generateAllProjects();
    ...
    const projects = allProjects.reduce(
        (pages, entry) =>
            Object.assign({}, pages, {
                [`/projects/${entry.slug}`]: {
                    page: '/projects',
    ...
    const entries = { ...defaults, ...articles, ...projects };
